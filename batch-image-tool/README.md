...
- **Pillow** and **Flask** (installed from `requirements.txt`)

> This project is intended for **Windows** and uses Windows-style paths and batch files.

## Setup (one time)

**Easiest:** Double‑click **`setup.bat`** in the project folder. It creates a virtual environment and installs dependencies. Run it once (or after pulling updates that change dependencies).

**Manual (Windows, PowerShell or `cmd`):**
...
You can run either the **web UI** or the **desktop (tkinter) UI**.

### Using the launchers

| Launcher                               | What it does                                                                 |
|----------------------------------------|-------------------------------------------------------------------------------|
| **`Launch.bat`** or **`Launch-web.bat`** | Starts the **web UI** (opens `http://127.0.0.1:5000` in your browser). No tkinter needed. |
| **`Launch-tk.bat`**                    | Starts the **desktop (tkinter) window**. Requires tkinter.                   |

### From a terminal (with venv activated)

- **Web UI**: `python run.py` or `python run.py --web` or `python -m app_web`
- **Desktop UI**: `python run.py --tk` or `python -m gui`
...
- **Pad to target size (transparent PNG)** – Pad the image with fully transparent pixels (alpha=0) to reach a target resolution without stretching/cropping. Includes alignment options (left/center/right × top/center/bottom). When enabled, output is forced to PNG.
  ...
  - **Target by ratio (W:H)** – Provide a ratio like `1:1` or `3.14:7.2`. The tool computes the smallest integer canvas (in pixels) that fits the image while matching that aspect ratio, then pads with transparency.
